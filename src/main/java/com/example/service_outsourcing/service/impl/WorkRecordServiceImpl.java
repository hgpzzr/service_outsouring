...
//		String recordId = GenerateIdUtil.getRecordId(workRecordMapper);
	...
	public ResultVO updateWorkRecord(UpdateWorkRecordForm form) {
		WorkRecord workRecord = workRecordMapper.selectByPrimaryKey(form.getRecordId());
		if (workRecord == null) {
			return ResultVOUtil.error(ResultEnum.WORK_RECORD_NOT_EXIST_ERROR);
		}
		BeanUtils.copyProperties(form, workRecord);
		int update = workRecordMapper.updateByPrimaryKey(workRecord);
		if (update != 1) {
			return ResultVOUtil.error(ResultEnum.DATABASE_OPTION_ERROR);
		}
...
//		String achievementId = GenerateIdUtil.getAchievementId(achievementMapper);
	...
	public ResultVO updateAchievement(UpdateAchievementForm form) {
		Achievement achievement = achievementMapper.selectByPrimaryKey(form.getAchievementId());
		if (achievement == null) {
			return ResultVOUtil.error(ResultEnum.ACHIEVEMENT_NOT_EXIST_ERROR);
		}
		BeanUtils.copyProperties(form, achievement);
		int update = achievementMapper.updateByPrimaryKey(achievement);
		if (update != 1) {
			return ResultVOUtil.error(ResultEnum.DATABASE_OPTION_ERROR);
		}
		return ResultVOUtil.success("更新成功");
...
//		String evaluateId = GenerateIdUtil.getEvaluateId(evaluateMapper);
	...
	public ResultVO updateEvaluate(UpdateEvaluateForm form) {
		Evaluate evaluate = evaluateMapper.selectByPrimaryKey(form.getEvaluateId());
		if (evaluate == null) {
			return ResultVOUtil.error(ResultEnum.EVALUATE_NOT_EXIST_ERROR);
		}
		BeanUtils.copyProperties(form, evaluate);
		int update = evaluateMapper.updateByPrimaryKey(evaluate);
		if (update != 1) {
			return ResultVOUtil.error(ResultEnum.DATABASE_OPTION_ERROR);
		}
		return ResultVOUtil.success("更新成功");
...
//		String attendanceId = GenerateIdUtil.getAttendanceId(attendanceMapper);
	...
	public ResultVO updateAttendance(UpdateAttendanceForm form) {
		Attendance attendance = attendanceMapper.selectByPrimaryKey(form.getAttendanceId());
		if (attendance == null) {
			return ResultVOUtil.error(ResultEnum.ATTENDANCE_NOT_EXIST_ERROR);
		}
		BeanUtils.copyProperties(form, attendance);
		int update = attendanceMapper.updateByPrimaryKey(attendance);
		if (update != 1) {
			return ResultVOUtil.error(ResultEnum.DATABASE_OPTION_ERROR);
		}
		return ResultVOUtil.success("更新成功");
